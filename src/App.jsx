...
exports.handler = async (event) => {
  try {
    ...
    // 2b. Detecta se foi a oferta Black (R$49,90) — checa nome do produto/oferta E o valor cobrado,
    // já que o nome exato do campo pode variar. Ajuste aqui se o payload real vier diferente.
    const nomeProdutoOuOferta = String(
      payload?.Product?.product_name || payload?.product_name ||
      payload?.Subscription?.plan?.name || payload?.plan?.name || ""
    ).toLowerCase();
    const valorCobrado = Number(
      payload?.Commissions?.charge_amount ?? payload?.charge_amount ??
      payload?.product_price ?? payload?.amount ?? payload?.price ?? 0
    );
    const isBlack = nomeProdutoOuOferta.includes("black") || valorCobrado === 49.9 || valorCobrado === 4990;

    const rolesLiberadas = isBlack ? ["assinante", "assinante_black"] : ["assinante"];
    ...
    if (existing) {
      // 5a. Já tinha conta -> adiciona a(s) role(s) liberada(s)
      const updateResp = await fetch(`${process.env.IDENTITY_URL}/admin/users/${existing.id}`, {
        method: "PUT",
        headers: { ...authHeader, "Content-Type": "application/json" },
        body: JSON.stringify({
          app_metadata: { ...(existing.app_metadata || {}), roles: rolesLiberadas },
        }),
      });
      ...
      return { statusCode: 200, body: `Acesso liberado (${rolesLiberadas.join(", ")}) — conta já existia` };
    }

    // 5b. Não tinha conta -> cria e convida já com a(s) role(s) liberada(s)
    const inviteResp = await fetch(`${process.env.IDENTITY_URL}/admin/users`, {
      ...
      body: JSON.stringify({
        email,
        email_confirm: false,
        app_metadata: { roles: rolesLiberadas },
      }),
    });
    ...
    return { statusCode: 200, body: `Conta criada e convite enviado (${rolesLiberadas.join(", ")})` };
  } catch (err) {
